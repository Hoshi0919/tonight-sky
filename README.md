...
- 2026-09-19 至 09-22 的早期海报曾使用一版有缺陷的盈月暗影绘制（终结线循环方向写反，暗影铺满左半盘、等效 k=0.5）。该问题已修复，09-19/20/21/22 四张海报均已用修复后的模板重渲染；单点像素采样无法发现此类缺陷，现改用"受光起点 vs 理论终结线位置"结构化断言 + 循环方向回归锁定测试。
